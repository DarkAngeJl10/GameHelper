...
IniRead, The_VersionName, %config%, Bottle 2-5, VersionBottle 2-5
IniRead, CheckforUpdates, %config%, Bottle 2-5, CheckforUpdates
...
{
	The_VersionName := 0
	IniWrite, %The_VersionName%, %config%, Bottle 2-5, VersionBottle 2-5
}
if (CheckforUpdates == "ERROR" or CheckforUpdates == "")
{
	CheckforUpdates := 1
	IniWrite, %CheckforUpdates%, %config%, Bottle 2-5, CheckforUpdates
}
...
if (CheckforUpdates != 0) {
	...
	if (The_LatestVersion != "") 
	{
		if (The_VersionName != The_LatestVersion) 
		{
			...
			IfMsgBox Yes
			{
				filedelete, Bottle 2-5.ahk
				IniWrite, %The_LatestVersion%, %config%, Bottle 2-5, VersionBottle 2-5
