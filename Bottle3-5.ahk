...
IniRead, The_VersionName, %config%, Bottle 3-5, VersionBottle 3-5
IniRead, CheckforUpdates, %config%, Bottle 3-5, CheckforUpdates
...
{
	The_VersionName := 0
	IniWrite, %The_VersionName%, %config%, Bottle 3-5, VersionBottle 3-5
}
if (CheckforUpdates == "ERROR" or CheckforUpdates == "")
{
	CheckforUpdates := 1
	IniWrite, %CheckforUpdates%, %config%, Bottle 3-5, CheckforUpdates
}
...
if (CheckforUpdates != 0) {
	...
	if (The_LatestVersion != "") 
	{
		if (The_VersionName != The_LatestVersion) 
		{
			...
			IfMsgBox Yes
			{
				filedelete, Bottle 3-5.ahk
				IniWrite, %The_LatestVersion%, %config%, Bottle 3-5, VersionBottle 3-5
